...
def save_holding(holding: general.HoldingCreate, db: Session = Depends(get_db), user=Depends(get_current_db_user)):
...
def delete_holding(holding_id: int, db: Session = Depends(get_db), user=Depends(get_current_db_user)):
